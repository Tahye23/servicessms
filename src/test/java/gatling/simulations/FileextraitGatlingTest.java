...
public class FileextraitGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all fileextraits").get("/api/fileextraits").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new fileextrait")
                        .post("/api/fileextraits")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"fexidfile\": null" +
                                ", \"fexparent\": \"SAMPLE_TEXT\"" +
                                ", \"fexdata\": null" +
                                ", \"fextype\": \"SAMPLE_TEXT\"" +
                                ", \"fexname\": \"SAMPLE_TEXT\"" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_fileextrait_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created fileextrait").get("${new_fileextrait_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created fileextrait").delete("${new_fileextrait_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Fileextrait entity").exec(scn);
