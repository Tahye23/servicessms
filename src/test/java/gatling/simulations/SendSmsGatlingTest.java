...
public class SendSmsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all sendSms").get("/api/send-sms").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sendSms")
                        .post("/api/send-sms")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"msgdata\": \"SAMPLE_TEXT\"" +
                                ", \"sendateEnvoi\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sendSms_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created sendSms").get("${new_sendSms_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created sendSms").delete("${new_sendSms_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SendSms entity").exec(scn);
