...
public class OTPStorageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all oTPStorages").get("/api/otp-storages").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new oTPStorage")
                        .post("/api/otp-storages")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"otsdateexpir\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"isOtpUsed\": null" +
                                ", \"isExpired\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_oTPStorage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created oTPStorage").get("${new_oTPStorage_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created oTPStorage").delete("${new_oTPStorage_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OTPStorage entity").exec(scn);
