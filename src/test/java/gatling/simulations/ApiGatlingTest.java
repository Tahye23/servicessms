 ...
 */
public class ApiGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all apis").get("/api/apis").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new api")
                        ...
                        .body(
                            StringBody("{" + "\"apiNom\": \"SAMPLE_TEXT\"" + ", \"apiUrl\": \"SAMPLE_TEXT\"" + ", \"apiVersion\": 0" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_api_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created api").get("${new_api_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created api").delete("${new_api_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Api entity").exec(scn);
