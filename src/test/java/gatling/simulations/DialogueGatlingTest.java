...
public class DialogueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dialogues").get("/api/dialogues").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dialogue")
                        .post("/api/dialogues")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dialogueId\": 0" + ", \"contenu\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dialogue_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dialogue").get("${new_dialogue_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created dialogue").delete("${new_dialogue_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Dialogue entity").exec(scn);
