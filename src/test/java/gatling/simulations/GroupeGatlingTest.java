 ...
 */
public class GroupeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all groupes").get("/api/groupes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new groupe")
                        .post("/api/groupes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"grotitre\": \"SAMPLE_TEXT\"" + ", \"user\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_groupe_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created groupe").get("${new_groupe_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created groupe").delete("${new_groupe_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Groupe entity").exec(scn);
