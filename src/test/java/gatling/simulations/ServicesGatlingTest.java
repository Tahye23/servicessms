    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all services").get("/api/services").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new services")
                        .post("/api/services")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"api\": \"SAMPLE_TEXT\"" + ", \"nom\": \"SAMPLE_TEXT\"" + ", \"description\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_services_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created services").get("${new_services_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created services").delete("${new_services_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Services entity").exec(scn);
