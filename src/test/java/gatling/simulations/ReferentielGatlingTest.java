 ...
 */
public class ReferentielGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all referentiels").get("/api/referentiels").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new referentiel")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"refCode\": \"SAMPLE_TEXT\"" +
                                ", \"refRadical\": \"SAMPLE_TEXT\"" +
                                ", \"refFrTitle\": \"SAMPLE_TEXT\"" +
                                ", \"refArTitle\": \"SAMPLE_TEXT\"" +
                                ", \"refEnTitle\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_referentiel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created referentiel").get("${new_referentiel_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created referentiel").delete("${new_referentiel_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Referentiel entity").exec(scn);
