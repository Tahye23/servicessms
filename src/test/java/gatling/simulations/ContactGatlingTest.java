 ...
 */
public class ContactGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all contacts").get("/api/contacts").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new contact")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"conid\": 0" +
                                ", \"connom\": \"SAMPLE_TEXT\"" +
                                ", \"conprenom\": \"SAMPLE_TEXT\"" +
                                ", \"contelephone\": \"SAMPLE_TEXT\"" +
                                ", \"statuttraitement\": 0" +
                                "}"
                            )
                        )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contact").get("${new_contact_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contact").delete("${new_contact_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Contact entity").exec(scn);
