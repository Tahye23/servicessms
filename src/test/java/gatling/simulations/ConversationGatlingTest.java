...
public class ConversationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all conversations").get("/api/conversations").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new conversation")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"covdatedebut\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"covdatefin\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created conversation").get("${new_conversation_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created conversation").delete("${new_conversation_url}").headers(headers_http_authenticated))
                .pause(10)
        );
