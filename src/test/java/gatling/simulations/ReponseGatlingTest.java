 ...
 */
public class ReponseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reponses").get("/api/reponses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reponse")
                        .post("/api/reponses")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"repvaleur\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reponse").get("${new_reponse_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reponse").delete("${new_reponse_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Reponse entity").exec(scn);
