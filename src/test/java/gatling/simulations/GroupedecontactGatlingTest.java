...
public class GroupedecontactGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all groupedecontacts").get("/api/groupedecontacts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new groupedecontact")
                        .post("/api/groupedecontacts")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_groupedecontact_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created groupedecontact").get("${new_groupedecontact_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created groupedecontact").delete("${new_groupedecontact_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Groupedecontact entity").exec(scn);
