...
public class ChoixGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all choixes").get("/api/choixes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new choix")
                        .post("/api/choixes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"chovaleur\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_choix_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created choix").get("${new_choix_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created choix").delete("${new_choix_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Choix entity").exec(scn);
