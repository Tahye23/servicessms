...
public class EntitedetestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all entitedetests").get("/api/entitedetests").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entitedetest")
                        .post("/api/entitedetests")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"nombrec\": 0" +
                                ", \"chamb\": null" +
                                ", \"champdate\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entitedetest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created entitedetest").get("${new_entitedetest_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created entitedetest").delete("${new_entitedetest_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Entitedetest entity").exec(scn);
