...
import { sampleWithRequiredData } from '../extended-user.test-samples';
...
describe('ExtendedUser Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule, ExtendedUserComponent],
      providers: [
        {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.extendedUsers?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to extendedUserService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getExtendedUserIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getExtendedUserIdentifier).toHaveBeenCalledWith(entity);
